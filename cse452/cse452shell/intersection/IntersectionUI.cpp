...
void IntersectionUI::draw() {
    ...
    HitRecord hr = s->intersect(pE1, dir);
    drawHits(hr);
    endDrawing();
}
...
void IntersectionUI::changeShape( ShapesUI::ShapeType type )
{
	...
	case ShapesUI::SHAPE_CUBE:
		...
		s->newTess(5, 5);
		break;
	case ShapesUI::SHAPE_SPECIAL1:
		s = new SpecialShape1();
		s->newTess(5, 5);
		break;
	case ShapesUI::SHAPE_SPECIAL2:
		s = new SpecialShape2();
		s->newTess(10, 5);
		break;
	}
...
void IntersectionUI::writeTest() const {
    ...
    std::ofstream file("../Myintersections.txt");
    file.precision(4);
    ...
    for (int i = seed; i < (seed + 1638); i++) {
		...
		Shape* s0 = new Cube();
        cubeHr = s0->intersect(p, dir);
		s0 = new Cylinder();
        cylinderHr = s0->intersect(p, dir);
		s0 = new Cone();
        coneHr = s0->intersect(p, dir);
		s0 = new Sphere();
        sphereHr = s0->intersect(p, dir);
